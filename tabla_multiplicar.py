#archivo vacio
print("comandos git")
print("git add .") # agregar o versionar archivos ...
print("git commit -m ") #subir fuentes con un mensaje
print("git push -u origin master")#subir archivos a la web

numero = int(input("Ingrese un numero a saber su tabla de multiplicar"))

print("1 X ",numero," = ",1*numero)
print("2 X ",numero," = ",2*numero)
print("3 X ",numero," = ",3*numero)
print("4 X ",numero," = ",4*numero)
print("5 X ",numero," = ",5*numero)
print("6 X ",numero," = ",6*numero)
print("7 X ",numero," = ",7*numero)
print("8 X ",numero," = ",8*numero)
print("9 X ",numero," = ",9*numero)
print("10 X ",numero," = ",10*numero)

#https://github.com/learsixela/logicaDiurno.git